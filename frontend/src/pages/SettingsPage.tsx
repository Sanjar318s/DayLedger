...
const settingsTabs: { path: string; label: TranslationKey; icon: string; suffix?: TranslationKey; adminOnly?: boolean }[] = [
  ...
  { path: '/settings/admin', label: 'adminPanel', icon: '🛡️', adminOnly: true },
];
...
export default function SettingsPage() {
  ...
  const isAdmin = String(user?.public_id) === '99999';
  const visibleTabs = settingsTabs.filter(tab => !tab.adminOnly || isAdmin);

  // DEBUG
  console.log('[SettingsPage] user:', user?.public_id, 'isAdmin:', isAdmin, 'visibleTabs:', visibleTabs.map(t => t.path));

  const showDebug = user && (import.meta.env.DEV || import.meta.env.VITE_DEBUG === 'true');

  return (
    <div>
      {showDebug && user && (
        <div className="fixed bottom-4 right-4 z-50 bg-cyan-100 dark:bg-cyan-900/30 border border-cyan-400 rounded-lg p-3 text-sm font-mono text-cyan-800 dark:text-cyan-200 shadow-lg">
          <div>Public ID: <span className="bg-cyan-200 dark:bg-cyan-800 px-1 rounded">#{user.public_id}</span> (type: {typeof user.public_id})</div>
          <div>isAdmin: <span className={isAdmin ? 'text-green-600' : 'text-red-600'}> {isAdmin ? 'YES ✓' : 'NO ✗'}</span></div>
        </div>
      )}
